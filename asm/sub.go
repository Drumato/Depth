...
func semanticSub(c *Mnemonic) *Mnemonic {
	...
	case Reg64:
		...
		if _, ok := register64[c.ROperand.Reg]; ok {
			...
			errOut(e)
			sub := fmt.Sprintf("%x", SubRM)
			if len(sub) == 1 {
				sub = "0" + sub
			}
			c.Op.Code, e = strconv.ParseUint(fmt.Sprintf("%x%s%x", rex, sub, modrm2), 16, 64)
			errOut(e)
			return c
		}
		...
		case Imm8, Imm16, Imm32:
			...
			errOut(err)
			c.Op.Code, e = strconv.ParseUint(fmt.Sprintf("%x%x%x%x", rex, SubRMImm, modrm2, buf.Bytes()), 16, 64)
			errOut(e)
			return c
		}
	case Reg32:
